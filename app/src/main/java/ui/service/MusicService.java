...
import ui.receiver.AlarmReceiver;
...
//        }
    ...
    public void onPrepared(MediaPlayer mp) {
        ...
        mp.seekTo(MyApplication.getInstance().getCurrentPosition());
...
//        });
        /*获取本地广播实例*/
        manager = MyApplication.getInstance().getManager();
    }
...
//        Log.d(TAG, SUB + "sendMessage" + what);
    ...
    public class MusicBinder extends Binder {
        ...
        public void startPlay(int currentMusic, int currentPosition) {
            ...
            MyApplication.getInstance().setIsSelected(true);
            MyApplication.getInstance().setCurrentPosition(currentPosition);
            MyApplication.getInstance().setCurrentMusic(currentMusic);
...
//                Log.d(TAG, SUB + "startPlay：" + audioList.get(currentMusic).getTitle());
        ...
        public void preTrack() {
            ...
            //MyApplication.getInstance().getTextAudioIndex().setTextColor(color);
        }
...
//            return PLAY_MODE[currentMode];
        ...
        public void changeProgress(int progress) {
            if (mediaPlayer.isPlaying()) {
                ...
                startPlay(MyApplication.getInstance().getCurrentMusic(), progress);
            }
        }
        ...
        private void changeTrack(int action) {
            ...
            switch (currentMode) {
                case MODE_SINGLE_CYCLE: // 单曲循环
                    startPlay(MyApplication.getInstance().getCurrentMusic(), 0);
                    break;
